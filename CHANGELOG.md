...
### Added
- Restart gluetun now link in Settings > Network > TikTok: after saving or removing a WireGuard config, the panel can restart the gluetun container for you so the new credentials apply immediately. Requires the host's Docker socket mounted into the app container (optional, off by default; see the commented volume line in docker-compose.yml and the README note on what the socket grants); without it the panel keeps the old restart-it-yourself message

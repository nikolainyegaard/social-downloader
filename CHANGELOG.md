...
### Added
- TikTok traffic can be routed through an HTTP proxy, toggled in Settings > Accounts > TikTok: the browser, page fetches, and downloads all go through it while the web UI and the other platforms stay on the server's own connection. Built for pairing with a gluetun VPN container (see the README for a compose example) as an escape hatch when TikTok rate limits your IP. The TIKTOK_PROXY env var seeds the setting, and changes apply from the next browser session without a restart
- The gluetun container's WireGuard config can be pasted directly into the same settings panel: the app saves it under its data folder where gluetun's volume reads it, so setting up or rotating VPN credentials needs no shell access. The saved config is write-only in the UI (only the endpoint and save time are shown back), and gluetun picks it up on its next container restart

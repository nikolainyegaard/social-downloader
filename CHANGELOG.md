...
### Added
- Live view of the TikTok browser: the container now serves the virtual display over noVNC on port 6080, so you can watch a running session in your own browser and solve a captcha or verification wall by hand with real mouse input. It is bound to localhost by default, so reach it through an SSH tunnel or put it behind an authenticated reverse proxy

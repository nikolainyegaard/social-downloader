"""Twitter download loop and shared state."""
...
from platforms.twitter.database import TWITTER_DATA_DIR

LOOP_STATE_PATH       = os.path.join(TWITTER_DATA_DIR, "loop_state.json")
LOOP_INTERVAL_MINUTES = int(os.environ.get("TWITTER_LOOP_INTERVAL_MINUTES", 180))
...
def run_loop() -> None:
    """Process all enabled tracked Twitter channels. Called by the scheduler thread."""
    ...
    if not channels:
        ...
        try:
            ...
            _log(f"Unhandled Twitter loop error: {e}")
    ...
    _log(f"=== Twitter loop complete: {_completed}/{len(channels)} channel(s), {new_videos} new video(s) ===")
    with _state_lock:

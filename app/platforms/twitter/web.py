"""Twitter Flask Blueprint: all /api/twitter/* routes."""
...
from platforms.twitter.api import normalize_handle
from platforms.twitter.loop import (
...
threading.Thread(target=_add_worker, daemon=True, name="tw-add-worker").start()
...
def _run_cleanup() -> None:
    ...
    try:
        ...
        thumb_count = 0
        for thumbs_dir in _glob.glob(os.path.join(MEDIA_DIR, "twitter", "*", "thumbs")):
...
@twitter_bp.route("/channels", methods=["GET"])
def list_channels():
...
@twitter_bp.route("/channels", methods=["POST"])
def add_channel():
...
@twitter_bp.route("/queue", methods=["GET"])
def get_queue():
    with _pending_lock:
        return jsonify(dict(_pending))


@twitter_bp.route("/queue/<handle>", methods=["DELETE"])
...
@twitter_bp.route("/channels/<channel_id>", methods=["DELETE"])
def remove_channel(channel_id: str):
    db.remove_channel(channel_id)
    return jsonify({"ok": True})


@twitter_bp.route("/channels/<channel_id>/videos", methods=["GET"])
def channel_videos(channel_id: str):
    return jsonify(db.get_videos_for_channel(channel_id))


@twitter_bp.route("/channels/<channel_id>/run", methods=["POST"])
...
@twitter_bp.route("/channels/<channel_id>/run-profile", methods=["POST"])
...
@twitter_bp.route("/channels/<channel_id>/tracking", methods=["PATCH"])
...
@twitter_bp.route("/channels/<channel_id>/star", methods=["PATCH"])
...
@twitter_bp.route("/channels/<channel_id>/comment", methods=["PATCH"])
...
@twitter_bp.route("/channels/<channel_id>/profile-history", methods=["GET"])
...
@twitter_bp.route("/channels/<channel_id>/avatar", methods=["GET"])
def channel_avatar(channel_id: str):
    from config import DATA_DIR
    path = os.path.join(DATA_DIR, "twitter", "avatars", f"{channel_id}.avif")
    if os.path.exists(path):
        return send_file(path, mimetype="image/avif")
    return ("", 404)


@twitter_bp.route("/channels/<channel_id>/banner", methods=["GET"])
def channel_banner(channel_id: str):
    from config import DATA_DIR
    path = os.path.join(DATA_DIR, "twitter", "banners", f"{channel_id}.avif")
    if os.path.exists(path):
        return send_file(path, mimetype="image/avif")
    return ("", 404)
...
@twitter_bp.route("/videos/<video_id>/thumbnail", methods=["GET"])
...
@twitter_bp.route("/videos/<video_id>/file", methods=["GET"])
...
@twitter_bp.route("/db/query", methods=["POST"])
def db_query():
    body = request.get_json(silent=True) or {}
...
@twitter_bp.route("/stats", methods=["GET"])
def get_aggregate_stats():
    return jsonify(db.get_aggregate_stats())


@twitter_bp.route("/recent", methods=["GET"])
def get_recent():
    return jsonify(db.get_recent_activity())


@twitter_bp.route("/recent/deletions", methods=["GET"])
...
@twitter_bp.route("/recent/profile-changes", methods=["GET"])
...
@twitter_bp.route("/recent/saved", methods=["GET"])
...
@twitter_bp.route("/db/cleanup", methods=["GET"])
def get_cleanup_status():
    with _cleanup_lock:
        return jsonify(dict(_cleanup_state))


@twitter_bp.route("/db/cleanup", methods=["POST"])
def start_cleanup():
    with _cleanup_lock:
        if _cleanup_state["running"]:
            return jsonify({"error": "Already running"}), 409
    threading.Thread(target=_run_cleanup, daemon=True, name="tw-db-cleanup").start()
    return jsonify({"ok": True})


# Loop API

@twitter_bp.route("/status", methods=["GET"])
def get_status():
...
@twitter_bp.route("/trigger/next", methods=["POST"])
...
@twitter_bp.route("/trigger", methods=["POST"])
def trigger_now():
    issues, running = _check_trigger_preconditions()
    if issues:
        return jsonify({"error": issues[0]["message"]}), 503
...
@twitter_bp.route("/trigger/half", methods=["POST"])
...
@twitter_bp.route("/trigger/all", methods=["POST"])
def trigger_all_now():
    issues, running = _check_trigger_preconditions()
    if issues:
        return jsonify({"error": issues[0]["message"]}), 503
...
@twitter_bp.route("/stop", methods=["POST"])
def stop_loop():
    if not is_running():
...
@twitter_bp.route("/settings", methods=["GET"])
def get_settings():
    from scheduling import platform_defaults
    defaults = platform_defaults("twitter")
    return jsonify({
        key: int(db.get_setting(key, defaults[key])) for key in _SCHEDULE_KEYS
    })


@twitter_bp.route("/settings", methods=["PATCH"])
def update_settings():
    body    = request.get_json(silent=True) or {}
    changed = False

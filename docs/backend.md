...
- **`start_backfill()`**: walks MEDIA_DIR in a thread and INSERT OR IGNOREs every qualifying mp4; largest files dequeue first. Also requeues rows skipped as `file missing` whose file exists again, keeping their recorded `orig_bytes`
- **`skip_current()`**: kills the in-flight ffmpeg (encode or VMAF pass); the file lands as failed with reason `cancelled` so it stays parked until Retry failed. Panel button appears while a file is processing
- **`remote` status**: a row claimed for processing outside the app (set directly in the DB); the worker ignores it. A pending file that probes as AV1 smaller than its recorded `orig_bytes` is credited as done (`transcoded externally`) instead of skipped; see the transcoder.py docstring
- **`min_bpp` gate**: before encoding, the worker computes the source's bits per pixel per frame (size*8/duration over width*height*fps) and skips files below the threshold (default 0.10, 0 disables) as "source already compact". Low-bitrate platform downloads (TikTok ~0.02-0.07 bpp) come out of a CRF 22 AV1 encode larger than they went in, so this replaces a wasted full encode with one ffprobe; healthy encode candidates measure ~0.28 bpp
...
- Deletion model (all platforms): `mark_video_possibly_deleted` (first absence: status='deleted', deletion_confirmed=0, deleted_at=now), `confirm_video_deletion` (second consecutive absence sets deletion_confirmed=1), `revert_or_undelete_video` (returns 'reverted' for an unconfirmed false positive, bumping false_positive_count, or 'undeleted' for a confirmed recovery). The old engine counter model (pending_deletion_count/since) is retired; the columns remain, unused. `_migrate_db` runs a one-time settings-gated backfill setting deletion_confirmed=1 on existing deleted rows for non-TikTok platforms (TikTok always maintained the flag, and its unconfirmed rows are genuinely pending)
- Also `record_profile_change`, `get_recent_activity`, `get_saved_history`, `backfill_upload_dates()`, `vacuum()` (raw connect, outside any transaction)
...
`process_all_channels(engine, ...)` / `process_single_channel(engine, ...)`. Quick/full cadence, deletion spike guard, TikTok-style two-strike deletion (full diffs mark first absences possibly deleted and confirm ones still absent; any listing that shows a deleted video again reverts or undeletes it, quick mode included), 3-failure session abort, profile change detection (falsy fetched fields count as missing, not changes), avatar/banner caching with the engine db as `db_obj`. Ban lifecycle: a `ChannelGoneError` marks the channel banned and its active videos deleted with reason user_banned; a later successful fetch restores them, re-enables tracking, and sets status active; 14+ consecutive banned days disable tracking once. Transient fetch errors only log. The inter-creator gap sleep reports through `set_sleep` (dropped while an inserted manual run executes, resumed with the remaining time), so the activity strip shows "sleeping X, up next: @handle" instead of the bare "session running"; TikTok's own session tracker does the same. Also `scan_afflicted_stories` / `purge_afflicted_stories` (story recovery).

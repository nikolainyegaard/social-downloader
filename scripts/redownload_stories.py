...
The stories table does not store the media URL (TikTok signs them with a short
expiry, so a saved URL would be dead anyway). It does store the story_id, and a
story's page is reachable at https://www.tiktok.com/@{handle}/story/{id}, which
yt-dlp can fetch and download self-consistently. That is the one re-download path
that works without the browser session, so this tool handles video stories only;
photo stories have no page to re-fetch here and are left for the app's next check.

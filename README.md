...
The panel can do that restart for you: mount the host's Docker socket into the app container (uncomment the `/var/run/docker.sock` volume line in docker-compose.yml) and a **Restart gluetun now** link appears after saving or removing a config. Without the mount the link stays hidden and nothing else changes. Be aware that the socket gives the app container control over the host's Docker daemon; skip it if that tradeoff is not worth the convenience.
